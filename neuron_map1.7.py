...
for i in range(N):
    for j in range(int(1000 * ts[1])):
        if j in spike_times[i]:
            neuron_spikes[i].append(1)
        else:
            neuron_spikes[i].append(0)
...
for t in range(int(1000 * ts[1])):
    plt.imshow(image,extent=[-2200, 2200, -2200, 2200], aspect='auto')    
    for i in range(N):
        if neuron_spikes[i][t] == 1:
            plt.scatter(neurons[i][0], neurons[i][1], c='yellow', linewidths=10)
        elif neuron_spikes[i][t] == 0:
            plt.scatter(neurons[i][0], neurons[i][1], c='black', linewidths=10)
    camera.snap()
