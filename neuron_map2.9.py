...
timeframe = int(1000* ts[1])
brightness = 10

for i in range(N):
    for j in range(timeframe):
        if j in spike_times[i]:
            neuron_spikes[i][j] = brightness

  
for i in range(N):
    for j in range(timeframe):
        if neuron_spikes[i][j] == brightness:
            for k in range(15):
                if j + k < timeframe - 1:
                    neuron_spikes[i][j+k+1] = 0
            for x in range(brightness - 1):
                neuron_spikes[i][j-(x+1)] = brightness - (x+1)
            for y in range(brightness - 1):
                if j < timeframe - (y + 1):
                    neuron_spikes[i][j+(y+1)] = brightness - (y+1)
                else:
                    break
...
camera = Camera(fig)
color_increment= (1/brightness)

for t in range(int(1000 * ts[1])):
    plt.imshow(image,extent=[-2224, 2224, -2224, 2224], aspect='auto')
    plt.text(-1900, 1900, "t="+str(t)+'ms', fontsize=16)
    plt.axis('off')
    for i in range(N):
        for j in range(brightness + 1):
            if neuron_spikes[i][t] == brightness - j:
                plt.scatter(neurons[i][0], neurons[i][1], c=[(color_increment*(brightness-j),color_increment*(brightness-j),0)], linewidths=5)
                break
    camera.snap()
    print(str(t+1)+'/'+str(int(1000 * ts[1])))

anim = camera.animate()
anim.save(url+'network_animation34.gif', writer=animation.PillowWriter(fps=30))
