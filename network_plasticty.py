...
for i in range(N):
    for j in range(N):
        if i==0 and j==N-1: M1[i][j]=1
        elif i==N-1 and j==0: M1[i][j]=1
        elif j>=i+1 and j<=i+q/2: M1[i][j]=1
        elif j<=i-1 and j>=i-q/2: M1[i][j]=1
        else: M1[i][j]=0
...
def Freq(t):
    freq=[]
    for i in range(N):
        number_spikes=len(np.array(t_spike_arr[i])<t)
        if number_spikes!=0:
            freq.append(1/number_spikes)
        else: freq.append(0)
...
for i in range(1,len(sol.t)):
    v_hat = (1/N)*np.sum(sol.y[:N,:i],axis=0)
...
url = "c://Users/User/Desktop/Network_Anime/" # where you want to download the animations to
...
#    for j in t_spike_arr:
#        if sol.t[i]!=i:
#            line,=ax.scatter(Centre[:,0],Centre[:,1],c='green',linewidths=10)
#        else:
#            line,=ax.scatter(Centre[:,0],Centre[:,1],c='yellow',linewidths=10)

    
#    return line,

#anim = animation.FuncAnimation(fig,animate,frames=(len(sol.t)-1),interval=20, blit=True, repeat=False)
#plt.show()
